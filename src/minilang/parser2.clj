...
;(do-production :expression_statement [parse-expression (expect :semicolon)] token-seq))    
    (if (contains? primary-expression-types token-type)
      ...
      (if((= (next-token [token-seq])(:lparen))
           (do-production :expression [(expect :lparen) parse-expression (expect:rparen)] token-seq)
...
;Parse While Statement Macro
(defn parse-while-statement [token-seq]
  (do-production :while_statement [(expect :while) parse-primary parse-statement-list] token-seq))

;Parse If Statement Macro
(defn parse-if-statement [token-seq]
  (do-production :if_statement [(expect :if) parse-primary parse-statement-list] token-seq))
; Parse a statement.
;
; Parameters:
;   token-seq - the input token sequence
...
(defn parse-statement [token-seq]
  (let [[lexeme token-type] (next-token token-seq)]
    (case token-type
      ...
      :if (do-production :if_statement [parse-if-statement] token-seq)
      ; TODO: other types of statements can be added here!
      :while (do-production :while_statement [parse-while-statement] token-seq)
